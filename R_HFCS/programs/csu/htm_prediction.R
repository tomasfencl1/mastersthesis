...
# Age, age2, selfempl, married and female variables
df <- df %>%
  mutate(
    age = ra0300,
    age2 = age^2,
...
#barplot(table(df$ra0300), main = "Distribution of variable",
...
if (imputationModelChoice == 1) {
  ...
  for (sector in nace2_sectors) {
    ...
    df[[dummy_name]] <- ifelse(df$pe0400 == sector, 1, ifelse(is.na(df$pe0400), NA, 0))
  }
  #df_sec <- df %>% filter(!(pe0400==""))
  #sec_counts <- table(df_sec$pe0400)
  #print(sec_counts)
  #barplot(sec_counts, main = "Distribution of NACE2 (pe0400)",
  #        col = "lightblue", las = 2, xlab = "NACE2", ylab = "Count")
} else if (imputationModelChoice %in% c(2, 3, 4)) {
  # Own NACE2 grouping (Slacalek i. transforms NACE2 to NACE, ii. groups some codes, e.g. A,B; C,D,E; O,P,Q, we create own grouping)
  df <- df %>%
    mutate(xsecgroup = case_when(
      pe0400 %in% c("A") ~ "A", # Agriculture, forestry and fishing
    ...
    mutate(xsecgroup = factor(xsecgroup,
                              levels = c("A", "B,D,E", "C", "F", "G,H,I", "J", "K",
                                         "L", "M,N", "O,P,Q", "R,S,T,U")))
  #sectoral_categories <- sort(unique(df$xsecgroup[!is.na(df$xsecgroup)]))
  sectoral_categories <- levels(df$xsecgroup)
...
#  filter(age >= 15 & age <= 64) %>% # Only keep productive-age population
...
if (runLASSO == 1) {
  ...
  write.csv(coeff_df_htm_lasso, file.path(OUTPUT, paste0("csu/beta_", DBNAME, "_prob_htm_lasso_", imputationModelChoice, ifelse(imputationWeighted == 1, "wt", "uwt"), ".csv")), row.names = FALSE)
  ...
  # Save the final coefficients to a CSV file
  write.csv(coeff_df_phtm_lasso, file.path(OUTPUT, paste0("csu/beta_", DBNAME, "_prob_phtm_lasso_", imputationModelChoice, ifelse(imputationWeighted == 1, "wt", "uwt"), ".csv")), row.names = FALSE)
  ...
  if (top10Index == 1) {
    ...
    # Save the final coefficients to a CSV file
    write.csv(coeff_df_top10_lasso, file.path(OUTPUT, paste0("csu/beta_", DBNAME, "_prob_top10_lasso_", imputationModelChoice, ifelse(imputationWeighted == 1, "wt", "uwt"), ".csv")), row.names = FALSE)
  }
}
...
if (runBMA == 1) {
  ...
  # Save the final coefficients to a CSV file
  write.csv(coeff_df_htm_bma, file.path(OUTPUT, paste0("csu/beta_", DBNAME, "_prob_htm_bma_", imputationModelChoice, ifelse(imputationWeighted == 1, "wt", "uwt"), ".csv")), row.names = FALSE)
  ...
  # Save the final coefficients to a CSV file
  write.csv(coeff_df_phtm_bma, file.path(OUTPUT, paste0("csu/beta_", DBNAME, "_prob_phtm_bma_", imputationModelChoice, ifelse(imputationWeighted == 1, "wt", "uwt"), ".csv")), row.names = FALSE)
  ...
  if (top10Index == 1) {
    ...
    # Save the final coefficients to a CSV file
    write.csv(coeff_df_top10_bma, file.path(OUTPUT, paste0("csu/beta_", DBNAME, "_prob_top10_bma_", imputationModelChoice, ifelse(imputationWeighted == 1, "wt", "uwt"), ".csv")), row.names = FALSE)
  }
}
...
if (runBMAhg == 1) {
  ...
  # Save the final coefficients to a CSV file
  write.csv(coeff_df_htm_bmahg, file.path(OUTPUT, paste0("csu/beta_", DBNAME, "_prob_htm_bmahg_", imputationModelChoice, ifelse(imputationWeighted == 1, "wt", "uwt"), ".csv")), row.names = FALSE)
  ...
  # Save the final coefficients to a CSV file
  write.csv(coeff_df_phtm_bmahg, file.path(OUTPUT, paste0("csu/beta_", DBNAME, "_prob_phtm_bmahg_", imputationModelChoice, ifelse(imputationWeighted == 1, "wt", "uwt"), ".csv")), row.names = FALSE)
  ...
  if (top10Index == 1) {
    ...
    # Save the final coefficients to a CSV file
    write.csv(coeff_df_top10_bmahg, file.path(OUTPUT, paste0("csu/beta_", DBNAME, "_prob_top10_bmahg_", imputationModelChoice, ifelse(imputationWeighted == 1, "wt", "uwt"), ".csv")), row.names = FALSE)
  }
}
...
if (runProbit == 1) {
  ...
  # Save to CSV
  write.csv(coeff_df_htm, file.path(OUTPUT, paste0("csu/beta_", DBNAME, "_prob_htm_probit_", imputationModelChoice, ifelse(imputationWeighted == 1, "wt", "uwt"), ".csv")), row.names = FALSE)
  ...
  # Save to CSV
  write.csv(coeff_df_phtm, file.path(OUTPUT, paste0("csu/beta_", DBNAME, "_prob_phtm_probit_", imputationModelChoice, ifelse(imputationWeighted == 1, "wt", "uwt"), ".csv")), row.names = FALSE)
  ...
  if (top10Index == 1) {
    ...
    # Save to CSV
    write.csv(coeff_df_top10, file.path(OUTPUT, paste0("csu/beta_", DBNAME, "_prob_top10_probit_",  imputationModelChoice, ifelse(imputationWeighted == 1, "wt", "uwt"), ".csv")), row.names = FALSE)
  }
}
...
# Save and print the model comparison table
